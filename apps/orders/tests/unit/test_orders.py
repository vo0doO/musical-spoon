...
from orders.domain.model import Order, OrderStatuses, RefundDTO, TicketUpdateDTO
...
def test_can_add_or_delete_ticket_or_change_ticket_price_use_update_tickets(fake_order, update_data):
    update_data = [TicketUpdateDTO(**data) for data in update_data]
    fake_order.update_tickets(update_data)
    ...
    expected_events_ids = {dto.event_id for dto in update_data}

    assert event_ids == expected_events_ids

    for dto in update_data:
...
@pytest.mark.parametrize(
    ...
    [
        {'event_ids': [101]},
        {'event_ids': [101, 102]},
        {'event_ids': [101, 102, 999]},
    ],
)
def test_can_refund_selected_tickets_only_when_order_is_done(fake_order, refund_data):
    refund_data = RefundDTO(**refund_data)
    fake_order.order_status = OrderStatuses.DONE
    fake_order.refund_tickets(refund_data)

    refunded_ids = {ticket.event_id for ticket in fake_order.tickets if ticket.refunded}
    expected_ids = set(refund_data.event_ids).intersection({ticket.event_id for ticket in fake_order.tickets})
...
def test_cant_refund_if_order_not_done(fake_order, refund_data):
    refund_data = RefundDTO(**refund_data)
    fake_order.refund_tickets(refund_data)
